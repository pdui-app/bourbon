...
class ClassyVirtualReferencePoint(object):
    ...
    def __init__(self, keypoints, descriptors, referencePointYX, bounds, eye1, eye2):
        ...
        keypoints, self.descriptors = self.cropToBounds(
            keypoints, descriptors, bounds, eye1, eye2)
        ...
        self.keypointdata = []
        for kp in keypoints:
            vector = (self.reference[0] - kp.pt[0],
                      self.reference[1] - kp.pt[1])
...
#        return (keypoints, np.array(descriptors, dtype = np.float32).reshape((-1, self.rowsize)))
    ...
    def getReferencePoint(self, keypoints, descriptors, bounds, eye1, eye2, img=None):
        ...
        """
        keypoints, descriptors = self.cropToBounds(
            keypoints, descriptors, bounds, eye1, eye2)
        _, _, oldneighborLabels, olddistances = self.oldknn.findNearest(
            descriptors, 2)
        ...
        _, _, newneighborLabels, newdistances = newknn.findNearest(
            self.descriptors, 2)
        #import pdb;
            ...
            if closestDistance < other * .7:
                ...
                if int(newneighborLabels[oldlabel][0]) == i:
                    ...
                    if closestDistance < other * .7:
                        ...
                        self.keypointdata[oldlabel].weight = min(
                            UPPER, self.keypointdata[oldlabel].weight + UP_STEP)
                        # pdb.set_trace()
                        self.keypointdata[oldlabel].guess = keypoints[i].pt[0] + self.keypointdata[oldlabel].vector[0], \
                            keypoints[i].pt[1] + \
                            self.keypointdata[oldlabel].vector[1]
                        # if there's an image, draw on it
                        if img is not None:
                            # this is different from my other code but I think it's good
                            cx, cy = keypoints[i].pt
                            cx, cy = int(cx), int(cy)
                            cv2.putText(img, str(oldlabel), (cx, cy),
                                        cv2.FONT_HERSHEY_SIMPLEX, .25, (100, 170, 0))
        # step down the weights of the not found ones
        for kp in self.keypointdata:
            if kp.found == 0:
                kp.weight = 0
        # if we've found at least 1 keypoint, recalculate the virtual reference
        if max([kp.weight for kp in self.keypointdata]) > 0:
